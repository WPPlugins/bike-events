...
Description: Displays a listing of upcoming and nearby bicycle rides, races and events based on the web users location or a zip code.  
Author: FreeHub
Version: 2.1
...
class BikeEventsWidget extends WP_Widget
{
  ...
  {
    $instance = wp_parse_args( (array) $instance, array( 'title' => 'Upcoming Cycling Events', 'zip' => '10001', 'max_display' => '10', 'distance' => '100', 'links' => '', 'use_ip' => '1' ) );
    $title = $instance['title'];
    $zip = $instance['zip'];
    $max_display = $instance['max_display'];
    $distance = $instance['distance'];
    $links = $instance['links'];
    $use_ip = $instance['use_ip'];
?>
  ...
  <p><label for="<?php echo $this->get_field_id('use_ip'); ?>">Show events near user's IP address <input id="<?php echo $this->get_field_id('use_ip'); ?>" name="<?php echo $this->get_field_name('use_ip'); ?>" type="checkbox" value="1" <?php checked( '1', attribute_escape($use_ip) ); ?>  /></label></p>
  <p><label for="<?php echo $this->get_field_id('zip'); ?>">Or show events near Zip Code: <input id="<?php echo $this->get_field_id('zip'); ?>" size="6" name="<?php echo $this->get_field_name('zip'); ?>" type="text" value="<?php echo attribute_escape($zip); ?>" /></label></p>
...
<?php
  ...
  function update($new_instance, $old_instance)
  {
    ...
    $instance['use_ip'] = $new_instance['use_ip'];
    return $instance;
  }
  ...
  function widget($args, $instance)
  {
	...
	echo '<input type="hidden" id="bike-events-use_ip" value="' . $use_ip . '"/>';
    ...
    echo '<span id="bike-events-spinner"><img src="/wp-content/plugins/bike-events/spinner.gif"/></span>';
    echo '<ul id="bike-events"></ul>';
    echo '<div id="bike-events-go"></div>';
    
    echo '<p><small><a href="http://freehub.net">FreeHub | Bike Rides, Races & Events</a></small></p>'; 
 	
 	echo '</div>';
    echo $after_widget;
    
    wp_enqueue_script("jquery");
	wp_enqueue_script( 'bike_events', '/wp-content/plugins/bike-events/bike_events.js', array('jquery') ); 
